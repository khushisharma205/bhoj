...
export default function Best_seller() {
  ...
  useEffect(() => {
    fetch("http://localhost:5001/api/product/all/best_seller")
      .then((res) => res.json())
      .then((data) => {
      ...
      <div className="flex justify-between items-center mb-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            ${view === 1 ? "bg-white text-black" : "text-gray-400"}`}
          >
          ...
          <button
            onClick={() => setView(2)}
            className={`border w-6 h-6 flex items-center justify-center
            ${view === 2 ? "bg-white text-black" : "text-gray-400"}`}
          >
            <div className="flex gap-[3px]">
              <span className="w-[4px] h-5 bg-current"></span>
              <span className="w-[4px] h-5 bg-current"></span>
            </div>
          </button>
          ...
          <button
            onClick={() => setView(3)}
            className={`border w-6 h-6 flex items-center justify-center
            ${view === 3 ? "bg-white text-black" : "text-gray-400"}`}
          >
            <div className="flex gap-[3px]">
              <span className="w-[4px] h-5 bg-current"></span>
              <span className="w-[4px] h-5 bg-current"></span>
          ...
          <button
            onClick={() => setView(4)}
            className={`border w-6 h-6 flex items-center justify-center
            ${view === 4 ? "bg-white text-black" : "text-gray-400"}`}
          >
